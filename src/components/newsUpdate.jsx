...
export default function NewsUpdate() {
  ...
  const [imageIndex, setImageIndex] = useState(0);
  const [isHovering, setIsHovering] = useState(false);
  const textSlides = chunkArray(newsData, 1); // One item per slide
...
return (
  <section
    className="relative w-full bg-gradient-to-br from-[#2c2c2c] to-[#1a1a1a] py-14 md:py-20"
    onMouseEnter={() => setIsHovering(true)}
    onMouseLeave={() => setIsHovering(false)}
  >
    {/* ARROWS */}
    <button
      onClick={prevSlide}
      className="absolute top-1/2 left-4 md:left-10 z-20 bg-white/60 rounded-full p-2 md:p-3 hover:bg-orange-500 hover:text-white transition"
    >
    ...
    <button
      onClick={nextSlide}
      className="absolute top-1/2 right-4 md:right-10 z-20 bg-white/60 rounded-full p-2 md:p-3 hover:bg-orange-500 hover:text-white transition"
    >
    ...
    <div className="max-w-7xl mx-auto px-4 md:px-8">
      <AnimatePresence mode="wait">
        ...
        >
          {/* TEXT */}
          <div className="w-full md:w-1/2 bg-black/70 rounded-2xl p-6 md:p-10 text-white">
            <h2 className="text-2xl md:text-4xl font-bold leading-tight">
              {newsData[contentIndex].title}
            </h2>
            <p className="mt-4 text-gray-300 text-sm md:text-lg">
              {newsData[contentIndex].text}
            </p>
          </div>

          {/* IMAGES */}
          <div className="w-full md:w-1/2 flex gap-4">
            {newsData[contentIndex].images.map((img, i) => (
              <div
                key={i}
